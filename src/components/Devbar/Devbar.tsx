...
const Devbar = () => {
  ...
  const [currentModule, setCurrentModule] = useState<string>(
    getItem('airbnb_react_module') || INITIAL_MODULE,
  );

  // Creates state for the current module progress and defaults to localStorage if it exists
  const [moduleProgress, setModuleProgress] = useState<Record<string, number>>(
    getItem('airbnb_react_moduleProgress') || initialModuleProgress,
  );

  // On mount, sets the initial module and module progress in localStorage if it doesn't exist
  useEffect(() => {
    if (!getItem('airbnb_react_module')) {
      setItem('airbnb_react_module', INITIAL_MODULE);
    }

    if (!getItem('airbnb_react_moduleProgress')) {
      setItem('airbnb_react_moduleProgress', initialModuleProgress);
    }
  }, []);
